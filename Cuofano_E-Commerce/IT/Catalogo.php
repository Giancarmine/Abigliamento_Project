	...
	<div id="main">
		...
		<div id="content">
			...
			--------------------------------------------------------------------------------------->
				...
				$con = mysqli_connect("127.0.0.1","root","","Cuofano_ECommerce") or die("Some error occurred during connection " . mysqli_error($con));  

				// Write query

				$strSQL = "SELECT	A.NomeArticolo , T.NomeTaglia , A.PrezzoArticolo , A.Immagine , M.NomeMateriale , Col.NomeColore , Cat.NomeCategoria , S.NomeSconto "
				...
				while($riga = mysqli_fetch_array($query))
				{
				  echo "<div class= 'element ". $riga['NomeCategoria'] ."'>".
								 ...
								 "<h1>" . $riga['NomeArticolo'] .  "</h1>".
								 "<p>Pezzo: " . $riga['PrezzoArticolo'] . " &#8364 Sconto: " . $riga['NomeSconto'] . " Colore: ".$riga['NomeColore']." Taglia: ".$riga['NomeTaglia']." Materiale: ".$riga['NomeMateriale']."</p>".
								 "</div>".
